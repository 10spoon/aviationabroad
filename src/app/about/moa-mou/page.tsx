...
const agreements = [
  {
    partner: "Jocson College", type: "MOA", year: "2022", location: "필리핀 클락",
  ...
  {
    partner: "Gentry International Technical College (GITC)", type: "MOA", year: "2022", location: "필리핀",
    ...
    <>
      ...
      <section className="py-16 px-4 sm:px-6 bg-white">
        <div className="max-w-4xl mx-auto space-y-5">
          {agreements.map((ag) => (
            <div key={ag.partner} className="group rounded-2xl bg-white border border-slate-900/[0.08] shadow-sm p-6 md:p-8 hover:shadow-md hover:border-sky-200 transition-all duration-300">
              <div className="flex flex-wrap items-start justify-between gap-4 mb-5">
                <div>
                  ...
                  <h3 className="text-slate-900 font-semibold text-xl tracking-tight">{ag.partner}</h3>
              ...
              <div className="flex flex-wrap gap-2">
                {ag.highlights.map((h) => (
                  <span key={h} className="px-3 py-1.5 bg-slate-50 border border-slate-900/[0.06] text-slate-500 text-xs rounded-lg">{h}</span>
                ))}
              </div>
            </div>
          ))}
